...
class UtilsTest(TestCase):
    ...
    def test_check_overlap_true(self):
        ...
        bus_shift = BusShiftFactory(arrival_time=arrival_time,
                                     departure_time=departure_time,
                                     driver=self.driver)

        driver_shifts = self.driver.shifts_driver.exclude(pk=bus_shift.id)
        departure = datetime.strptime(bus_shift.departure_time.transit_time, "%H:%M:%S").time()
        arrival = datetime.strptime(bus_shift.departure_time.transit_time, "%H:%M:%S").time()
